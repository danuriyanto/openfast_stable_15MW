function writeHydroDyn_v352(HydroDyn)
...
C(end+1,1) = {['0 ExctnMod - Wave-excitation model {0: no wave-excitation calculation, 1: DFT, 2: state-space} (switch) [only used when PotMod=1; STATE-SPACE REQUIRES *.ssexctn INPUT FILE]	']};
C(end+1,1) = {['0 ExctnDisp - Method of computing Wave Excitation {0: use undisplaced position, 1: use displaced position, 2: use low-pass filtered displaced position) [only used when PotMod=1 and ExctnMod>0 and SeaState"s WaveMod>0]} (switch)	']};
C(end+1,1) = {['10 ExctnCutOff - Cutoff (corner) frequency of the low-pass time-filtered displaced position (Hz) [>0.0] [used only when PotMod=1, ExctnMod>0, and ExctnDisp=2]) [only used when PotMod=1 and ExctnMod>0 and SeaState"s WaveMod>0]} (switch)	']};
...
C(end+1,1) = {['10 RdtnTMax - Analysis time for wave radiation kernel calculations (sec) [only used when PotMod=1 and RdtnMod>0; determines RdtnDOmega=Pi/RdtnTMax in the cosine transform; MAKE SURE THIS IS LONG ENOUGH FOR THE RADIATION IMPULSE RESPONSE FUNCTIONS TO DECAY TO NEAR-ZERO FOR THE GIVEN PLATFORM!]	']};
...
C(end+1,1) = {['0 NBody - Number of WAMIT bodies to be used (-) [>=1; only used when PotMod=1. If NBodyMod=1, the WAMIT data contains a vector of size 6*NBody x 1 and matrices of size 6*NBody x 6*NBody; if NBodyMod>1, there are NBody sets of WAMIT data each with a vector of size 6 x 1 and matrices of size 6 x 6]	']};
C(end+1,1) = {['0 NBodyMod - Body coupling model {1: include coupling terms between each body and NBody in HydroDyn equals NBODY in WAMIT, 2: neglect coupling terms between each body and NBODY=1 with XBODY=0 in WAMIT, 3: Neglect coupling terms between each body and NBODY=1 with XBODY=/0 in WAMIT} (switch) [only used when PotMod=1]	']};
...
C(end+1,1) = {['13917 PtfmVol0 - Displaced volume of water when the body is in its undisplaced position (m^3) [1 to NBody] [only used when PotMod=1; USE THE SAME VALUE COMPUTED BY WAMIT AS OUTPUT IN THE .OUT FILE!]	']};
...
C(end+1,1) = {['0 0 0 0 0 0	']};
C(end+1,1) = {['0 0 0 0 0 0	']};
C(end+1,1) = {['---------------------- STRIP THEORY OPTIONS --------------------------------------	']};
C(end+1,1) = {['1 WaveDisp - Method of computing Wave Kinematics {0: use undisplaced position, 1: use displaced position) } (switch)	']};
C(end+1,1) = {['0 AMMod - Method of computing distributed added-mass force. (0: Only and always on nodes below SWL at the undisplaced position. 2: Up to the instantaneous free surface) [overwrite to 0 when WaveMod = 0 or 6 or when WaveStMod = 0 in SeaState]	']};
C(end+1,1) = {['---------------------- AXIAL COEFFICIENTS --------------------------------------	']};
C(end+1,1) = {['2 NAxCoef - Number of axial coefficients (-)	']};
C(end+1,1) = {['AxCoefID AxCd AxCa AxCp AxFDMod AxVnCOff AxFDLoFSc	']};
C(end+1,1) = {['(-) (-) (-) (-) (-) (-) (-)	']};
C(end+1,1) = {['1 0.00 0.00 1.00 0 0.0 1.0	']};
C(end+1,1) = {['2 9.60 0.00 1.00 0 0.0 1.0	']};
...
C(end+1,1) = {['SimplCd SimplCdMG SimplCa SimplCaMG SimplCp SimplCpMG SimplAxCd SimplAxCdMG SimplAxCa SimplAxCaMG SimplAxCp SimplAxCpMG SimplCb SimplCbMG	']};
C(end+1,1) = {['(-) (-) (-) (-) (-) (-) (-) (-) (-) (-) (-) (-) (-) (-) 	']};
...
C(end+1,1) = {['MemberID MJointID1 MJointID2 MPropSetID1 MPropSetID2 MDivSize MCoefMod MhStlMod PropPot [MCoefMod=1: use simple coeff table, 2: use depth-based coeff table, 3: use member-based coeff table] [ PropPot/=0 if member is modeled with potential-flow theory]	']};
C(end+1,1) = {['(-) (-) (-) (-) (-) (m) (switch) (switch) (flag)	']};
C(end+1,1) = {['1 1 2 1 1 0.5 1 1 FALSE 	']};
